...
pub enum Commands {
    ...
    /// Inspect and control durable email notification delivery.
    Notifications {
        #[command(subcommand)]
...
pub enum NotificationCommand {
    /// List locally persisted notification events.
    List {
        #[arg(long, default_value_t = false)]
        json: bool,
    },
    /// Retry a failed or acknowledged notification event immediately.
    Retry { key: String },
    /// Acknowledge an event and suppress further automatic delivery.
    Acknowledge { key: String },
    /// Attempt delivery of all due events now.
    Deliver,
}
