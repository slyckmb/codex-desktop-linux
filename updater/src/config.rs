...
impl RuntimeConfig {
    ...
    pub fn load_or_default(paths: &RuntimePaths) -> Result<Self> {
        ...
        // This is an override file, not a complete serialized runtime state.
        // In particular, optional features such as email must not require a
        // user to duplicate machine-specific builder and cache paths.
        let defaults = toml::to_string(&Self::default_with_paths(paths))
            .context("Failed to serialize default updater configuration")?;
        let mut merged = toml::from_str::<toml::Value>(&defaults)
            .context("Failed to prepare default updater configuration")?;
        let supplied = toml::from_str::<toml::Value>(&content)
            .with_context(|| format!("Failed to parse {}", paths.config_file.display()))?;
        merge_toml_value(&mut merged, supplied)?;
        let mut config = toml::from_str::<Self>(&toml::to_string(&merged)?)
            .with_context(|| format!("Failed to parse {}", paths.config_file.display()))?;
...
fn merge_toml_value(defaults: &mut toml::Value, supplied: toml::Value) -> Result<()> {
    let (toml::Value::Table(defaults), toml::Value::Table(supplied)) = (defaults, supplied) else {
        anyhow::bail!("Updater configuration must be a TOML table");
    };
    for (key, value) in supplied {
        match (defaults.get_mut(&key), value) {
            (Some(default), value @ toml::Value::Table(_)) if default.is_table() => {
                merge_toml_value(default, value)?;
            }
            (_, value) => {
...
"#
    ...
    #[test]
    fn partial_config_overrides_defaults_without_requiring_runtime_paths() -> Result<()> {
        let temp = tempdir()?;
        let paths = RuntimePaths {
            config_file: temp.path().join("config/config.toml"),
            state_file: temp.path().join("state/state.json"),
