...
pub async fn build_update_from_with_feature_config(
    ...
    candidate_version: &str,
    dmg_path: &Path,
    feature_config_override: Option<&Path>,
) -> Result<BuildArtifacts> {
    ...
    state.save_updater(&paths.state_file)?;
    let feature_config = feature_config_override
        .map(Path::to_path_buf)
        .or_else(|| crate::config::effective_feature_config_path(config));
...
"#,
        ...
        let immutable_feature_snapshot = temp.path().join("state/feature-snapshots/candidate.json");
        fs::create_dir_all(immutable_feature_snapshot.parent().unwrap())?;
        fs::write(
            &immutable_feature_snapshot,
            r#"{"enabled":["snapshot-feature"]}"#,
        )?;
        std::env::set_var("CODEX_LINUX_SETTINGS_FILE", &settings_file);

        let mut state = PersistedState::new(true);
        let artifacts = runtime.block_on(build_update_from_with_feature_config(
            &config.builder_bundle_root,
            &config,
            &mut state,
            &paths,
            "2026.03.24+abcd1234",
            &dmg_path,
            Some(&immutable_feature_snapshot),
        ))?;
        ...
        for relative_path in [
            ...
            assert_eq!(
                ...
                format!("{}\n", immutable_feature_snapshot.display())
            );
        }
        assert!(artifacts
            .workspace_dir
