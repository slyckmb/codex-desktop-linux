...
use crate::{
    ...
    cli::{Cli, Commands, NotificationCommand},
    codex_cli,
    config::{RuntimeConfig, RuntimePaths},
    diagnostics, feature_picker, feature_snapshot, install, install_rollback, liveness, logging,
    notify, operational_guard, outbox, restart, rollback,
...
pub async fn run(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::CheckWrapper { json } => {
            run_check_wrapper(&config, &mut state, &paths, json)?;
            outbox::deliver_due(&paths).await?;
            Ok(())
        }
        Commands::Notifications { command } => run_notifications(&paths, command).await,
...
async fn run_daemon(
    ...
    if let Err(error) = outbox::deliver_due(paths).await {
        warn!(?error, "initial notification delivery failed");
    }
...
fn detect_and_record_wrapper_update(
    ...
    // Wrapper detection must not fetch into the packaged update-builder (or a
    // developer checkout). Use the same private managed source cache that a
    // later apply operation builds from. This also makes self-update detection
    // available to frozen native packages whose bundled source has no `.git`.
    let managed_source = match wrapper_apply::ensure_wrapper_source(config, paths, None) {
        Ok(path) => path,
        Err(error) => {
            warn!(
                ?error,
                "could not refresh managed wrapper source for update detection"
            );
            let original_state = state.clone();
            state.installed_wrapper_version = installed.version;
            state.installed_wrapper_commit = Some(installed.commit);
    ...
    match detection {
        (UpdateAvailable, Some(update)) => {
            feature_snapshot::capture(
                paths,
                &format!("wrapper:{}", update.candidate_commit),
                crate::config::effective_feature_config_path(config).as_deref(),
            )?;
            ...
            maybe_notify(
                ...
                &format!(
                    "A newer Linux wrapper build is available. Current commit: {}. Candidate commit: {}. Candidate version: {}. {change_count} changelog line(s) are available. Rebuild to apply.",
...
fn run_check_wrapper(
    ...
    if json {
        ...
        println!("wrapper is up to date (or managed source is unavailable).");
    }

    Ok(())
}

async fn run_notifications(paths: &RuntimePaths, command: NotificationCommand) -> Result<()> {
    match command {
        NotificationCommand::List { json } => {
            let events = outbox::list(paths)?;
            if json {
                println!("{}", serde_json::to_string_pretty(&events)?);
            } else if events.is_empty() {
                println!("No notification events.");
            } else {
                for event in events {
                    println!(
                        "{}\t{:?}\tattempts={}\t{}",
                        event.key, event.status, event.attempts, event.summary
                    );
                }
            }
        }
        NotificationCommand::Retry { key } => {
            outbox::retry(paths, &key)?;
            println!("Notification event queued for retry: {key}");
        }
        NotificationCommand::Acknowledge { key } => {
            outbox::acknowledge(paths, &key)?;
            println!("Notification event acknowledged: {key}");
        }
        NotificationCommand::Deliver => {
            let delivered = outbox::deliver_due(paths).await?;
            println!("Delivered {delivered} notification event(s).");
        }
    }
    Ok(())
}
...
async fn run_check_cycle_with_options(
    ...
    if let Some(reason) = operational_guard::deferral_reason(paths, &config.workspace_root)? {
        ...
        maybe_notify_with_event_key(
            ...
            paths,
            config.notifications,
            &format!("update_deferred:{}", reason),
            "ChatGPT Desktop update deferred",
            &format!("{reason}. The working app was not changed; the daemon will try again on its next scheduled check."),
        )?;
        info!(%reason, "deferring update check due to configured host guard");
        return Ok(());
    }
    ...
    if let Err(error) = result {
        ...
        state.consecutive_check_failures = state.consecutive_check_failures.saturating_add(1);
        mark_failed_and_persist(state, paths, error.to_string())?;
        if state.consecutive_check_failures >= 3 {
            ...
            outbox::enqueue(
                ...
                event_key,
                "check_unhealthy",
                "ChatGPT Desktop update checks are failing",
                "Three or more consecutive update checks have failed. The working app has not been changed. Run `codex-update-manager diagnose --json` and inspect the updater log.",
            )?;
        }
        let _ = notify_failure(config, state, paths, &error);
        return Err(error);
    }
    if state.consecutive_check_failures != 0 {
        state.consecutive_check_failures = 0;
        persist_state(paths, state)?;
    }
...
async fn run_install_ready_locked(
    ...
    let Some(package_path) = state.artifact_paths.package_path.clone() else {
        ...
        maybe_notify(
            ...
            paths,
            config.notifications,
            "install_failed_missing_package_path",
    ...
    if !package_path.exists() {
        ...
        maybe_notify(
            ...
            paths,
            config.notifications,
            "install_failed_missing_package",
            "ChatGPT Desktop update failed",
            "The rebuilt package is missing. The working app was not changed; run `codex-update-manager diagnose --json` and use `codex-update-manager rollback` if needed.",
        )?;
        println!(
...
fn maybe_notify_with_event_key(
    ...
    // Persist email intent before attempting the best-effort desktop surface.
    // Its stable event key gives the outbox exactly one durable record per
    // lifecycle transition, including when desktop notifications are disabled.
    let kind = event_key.split(':').next().unwrap_or("update_event");
    outbox::enqueue(paths, event_key, kind, summary, body)?;
...
fn maybe_notify_installed(
    ...
    maybe_notify(
        ...
        &format!(
            "Installed package: {}. The new package is ready for the next launch. If necessary, run `codex-update-manager rollback` to restore the retained known-good package.",
...
async fn trigger_install(
    ...
    outbox::enqueue(
        ...
        "install_failed",
        "ChatGPT Desktop update failed",
        "The package could not be installed and the previous working version remains available. Run `codex-update-manager diagnose --json`; if needed, run `codex-update-manager rollback` after closing the app.",
    )?;
    let _ = notify::send(
        "ChatGPT Desktop update failed",
...
fn notify_failure(
    ...
    let rejected = error_text.contains("candidate rejected")
        || error_text.contains("acceptance profile")
        || error_text.contains("acceptance verdict");
    let (event, summary, body) = if rejected {
        (
            "candidate_rejected",
            "ChatGPT Desktop update candidate rejected",
            format!(
                "Candidate {} (DMG SHA-256: {}) did not pass the shared acceptance profile and was not promoted. The working app remains installed. Run `codex-update-manager diagnose --json`; if a prior package is needed, run `codex-update-manager rollback`.",
                state.candidate_version.as_deref().unwrap_or("unknown"),
                state.dmg_sha256.as_deref().unwrap_or("unknown"),
            ),
        )
    } else {
        (
            "build_failed",
            "ChatGPT Desktop update failed",
            "The local rebuild failed and the working app was not replaced. Run `codex-update-manager diagnose --json` and inspect the updater log for the exact failure; use `codex-update-manager rollback` if needed.".to_string(),
        )
    };
    maybe_notify(state, paths, config.notifications, event, summary, &body)
}
...
"#,
    ...
    #[test]
    fn wrapper_detection_uses_private_managed_checkout_for_frozen_builder() -> Result<()> {
        let _environment = crate::test_util::env_lock();
        let temp = tempfile::tempdir()?;
        let paths = test_paths(temp.path());
        paths.ensure_dirs()?;
        let remote = temp.path().join("remote");
        std::fs::create_dir_all(remote.join("updater"))?;
        let git_bin = ["/usr/bin/git", "/bin/git", "/usr/local/bin/git"]
            .into_iter()
        ...
        let git = |args: &[&str]| -> Result<()> {
            let status = Command::new(git_bin)
                ...
                .status()
                .context("run test git")?;
            anyhow::ensure!(status.success(), "test git command failed: {args:?}");
            Ok(())
        };
        git(&["init", "-q", "-b", "main"])?;
        std::fs::write(
            remote.join("updater/Cargo.toml"),
            "[package]\nname = \"codex-update-manager\"\nversion = \"0.1.0\"\n",
        )?;
        std::fs::write(remote.join("CHANGELOG.md"), "# Changelog\n")?;
        git(&["add", "."])?;
        git(&["commit", "-q", "-m", "initial"])?;
        let installed_commit = String::from_utf8(
            Command::new(git_bin)
                .current_dir(&remote)
                .args(["rev-parse", "HEAD"])
                .output()?
        ...
        git(&["add", "."])?;
        git(&["commit", "-q", "-m", "new wrapper"])?;

        let mut config = test_config(temp.path());
        config.enable_wrapper_updates = true;
        config.wrapper_remote = remote.display().to_string();
        std::fs::create_dir_all(config.builder_bundle_root.join(".codex-linux"))?;
        std::fs::write(
            config
                .builder_bundle_root
                .join(".codex-linux/source-info.json"),
            format!("{{\"commit\":\"{installed_commit}\",\"version\":\"0.1.0\"}}"),
        )?;

        let mut state = PersistedState::new(true);
        assert!(detect_and_record_wrapper_update(
        ...
        assert!(!config.builder_bundle_root.join(".git").exists());
        Ok(())
    }
