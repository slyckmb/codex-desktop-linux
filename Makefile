...
.PHONY: help check test build-updater maybe-build-updater update rebuild rebuild-install inspect-upstream inspect-upstream-intel inspect-upstream-intel-devcontainer build-app build-app-fresh setup-native bootstrap-native install-native update-native rebuild-next run-app build-dev-app run-dev-app deb rpm pacman appimage package install service-enable service-status clean-dist clean-state sync-upstream fetch-upstream rebase-upstream push-origin assert-downstream-branch assert-clean-worktree
...
help:
	...
	@printf '  %-18s %s\n' "make sync-upstream" "On main: fetch upstream, rebase, then push with force-with-lease"
	@printf '  %-18s %s\n' "make fetch-upstream" "Fetch upstream remote (ilysenko/codex-desktop-linux)"
	@printf '  %-18s %s\n' "make rebase-upstream" "On main: rebase onto upstream/main (fetch first)"
...
sync-upstream:
	@$(MAKE) assert-downstream-branch
	@$(MAKE) assert-clean-worktree
	@$(MAKE) fetch-upstream
...
rebase-upstream:
	@$(MAKE) assert-downstream-branch
	@$(MAKE) assert-clean-worktree
	@echo "[make] Rebasing $(DOWNSTREAM_BRANCH) onto $(UPSTREAM_REMOTE)/$(UPSTREAM_BRANCH)"
	git rebase --rebase-merges "$(UPSTREAM_REMOTE)/$(UPSTREAM_BRANCH)"

push-origin:
	@$(MAKE) assert-downstream-branch
	@echo "[make] Pushing $(DOWNSTREAM_BRANCH) to origin with force-with-lease"
	git push --force-with-lease origin "$(DOWNSTREAM_BRANCH)"

assert-downstream-branch:
	@branch="$$(git branch --show-current)"; \
	if [ "$$branch" != "$(DOWNSTREAM_BRANCH)" ]; then \
		echo "[make] Refusing upstream sync from '$$branch'. Switch to '$(DOWNSTREAM_BRANCH)' first." >&2; \
		echo "[make] To rebase a feature branch after main is synced: git rebase $(DOWNSTREAM_BRANCH)" >&2; \
		exit 2; \
	fi

assert-clean-worktree:
	@if [ -n "$$(git status --porcelain)" ]; then \
		echo "[make] Refusing upstream rebase with uncommitted changes. Commit or stash them first." >&2; \
		exit 2; \
	fi
